"""Orbiteus DSV / DB Schenker adapter — native Python (no Mercato subprocess)."""
...
def _build_request_from_dispatch(payload: dict[str, Any]) -> ShipmentRequest:
    if payload.get("ifs_payload") and payload.get("packages"):
        return build_shipment_request_from_ifs(
            payload["ifs_payload"],
            payload.get("carrier_code") or "DSV",
            payload["packages"],
        )
    ...
    if parcels_raw:
        parcels = [
            ParcelInfo(
                ...
                width=p.get("width"),
                height=p.get("height"),
                pack_type=p.get("pack_type") or p.get("packType") or "EP",
    ...
    if payload.get("pickup_location") and "pickupLocation" not in opts:
        opts["pickupLocation"] = payload["pickup_location"]
    if payload.get("is_pallet") and not any(p.pack_type for p in parcels):
        for p in parcels:
            if not p.pack_type:
                p.pack_type = "EP"

    return ShipmentRequest(
        order_no=str(payload.get("reference") or payload.get("order_id") or "ORB-DSV"),
...
class DsvPythonAdapter:
    def __init__(self, code: str = "DSV") -> None:
        self.code = code.upper()
        self._carrier = DsvCarrier()
